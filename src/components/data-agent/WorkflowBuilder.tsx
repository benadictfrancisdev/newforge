...
const CONNECTOR_TEMPLATES: Omit<DataConnector, "id" | "status" | "config" | "lastSync">[] = [
  ...
  { name: "Tally Prime", type: "tally", icon: FileSpreadsheet, description: "Connect to Tally Prime — India's #1 accounting software" },
...
const WorkflowBuilder = ({ onDataLoaded }: WorkflowBuilderProps) => {
  ...
  const handleAddConnector = async () => {
    ...
    // Real connection test — call edge function for supported types
    ...
    toast({ title: '✅ Connected', description: `${newConnector.name} connected and verified.` });
  };
  ...
  const handleSyncConnector = async (connectorId: string) => {
    ...
    try {
      ...
      if (edgeTypes.includes(connector.type)) {
        ...
        toast({ title: '✅ Sync Complete', description: `${fetchedData.length.toLocaleString()} records imported from ${connector.name}.` });
  ...
  const renderConnectorForm = () => {
    ...
    const formFields: Record<string, JSX.Element> = {
      ...
      tally: (
        <div className="space-y-4">
          <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 mb-2">
            <p className="text-xs font-semibold text-primary mb-1">🇮🇳 Tally Prime Integration</p>
      ...
      zoho_books: (
        <div className="space-y-4">
          <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 mb-2">
            <p className="text-xs font-semibold text-primary mb-1">🇮🇳 Zoho Books Integration</p>
      ...
      busy: (
        <div className="space-y-4">
          <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 mb-2">
            <p className="text-xs font-semibold text-primary mb-1">🇮🇳 Busy Accounting Integration</p>
      ...
      vyapar: (
        <div className="space-y-4">
          <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 mb-2">
            <p className="text-xs font-semibold text-primary mb-1">🇮🇳 Vyapar Integration</p>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="p-4 rounded-xl border-2 border-primary/30 bg-primary/5">
                        ...
                        <p className="text-xs text-muted-foreground mb-3">
                          Connect directly to Tally Prime, Zoho Books, Busy & Vyapar — no CSV exports needed.
                        </p>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          {CONNECTOR_TEMPLATES.filter(t => ["tally", "zoho_books", "busy", "vyapar"].includes(t.type)).map((template) => {
                            const Icon = template.icon;
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex gap-3">
                        ...
                        <Button onClick={handleAddConnector} disabled={isConnecting} className="gap-2">
                          {isConnecting ? <><Loader2 className="w-4 h-4 animate-spin" />Testing connection…</> : <><CheckCircle2 className="w-4 h-4" />Test & Connect</>}
                        </Button>
                      </div>
                    </div>
                  )}
