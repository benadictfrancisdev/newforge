...
const NaturalLanguageEngine = ({ 
  ...
  const processQuery = async (query: string) => {
    ...
    try {
      ...
      if (!responseData?.success) throw new Error(responseData?.error || "Query failed");

      // Parse AI response and clean markdown
      let aiResponse = responseData.answer || "I've analyzed your data. Here's what I found...";
      aiResponse = aiResponse.replace(/\*\*/g, '');
      aiResponse = aiResponse.replace(/\*/g, '');
      aiResponse = aiResponse.replace(/#{1,6}\s*/g, '');
      aiResponse = aiResponse.replace(/`{1,3}/g, '');
      ...
      const confidence = (responseData.confidence || 0.95) * 100;
